...
MODEL_CONFIGS = {
    'R(2+1)D_26': {
        'target_layer': 'module.features.17'
    },
    'R(2+1)D_18': {
        'target_layer': 'module.features.13'
    },
    'R(2+1)D_16': {
        'target_layer': 'module.features.12'
    },
    'R(2+1)D_10': {
        'target_layer': 'module.features.9'
    },
    'VGG_19': {
...
def get_cams(args):
    ...
    config = MODEL_CONFIGS['R(2+1)D_{}'.format(model.module.args_dict()['model_depth'])]
    print('Last layer in model.features is named "{}"...'.format([k for k in model.module.features._modules.keys()][-1]))
    ...
    num_generated = 0
    data_loader = CTDataLoader(args, phase=args.phase, is_training=True)  # Change back to args.phase
    for inputs, labels in data_loader:
        ...
        video_np_cam = util.un_normalize(inputs[0], args.img_format, args.use_contrast)
        video_np_cam = np.transpose(video_np_cam, (1, 2, 3, 0))

        video_np_cam = np.float32(video_np_cam) / 255
        video_final_cam = list(util.add_heat_map(video_np_cam, new_cam))

        video_np_gbp = util.normalize_image(guided_backprop * new_cam)
        video_final_gbp = []
        for dim in range(video_np_gbp.shape[0]):
            slice_ = video_np_gbp[dim, :, :]
            video_final_gbp.append(slice_[..., None])

        # Write to a GIF file
        output_path_cam = os.path.join(args.cam_dir, 'cam_{}.gif'.format(num_generated + 1))
        output_path_gcam = os.path.join(args.cam_dir, 'gcam_{}.gif'.format(num_generated + 1))

        print('Writing CAM to {}...'.format(output_path_cam))
        clip = mpy.ImageSequenceClip(video_final_cam, fps=4)
        clip.write_gif(output_path_cam, verbose=False)
        print('Writing CAM to {}...'.format(output_path_gcam))
        clip = mpy.ImageSequenceClip(video_final_gbp, fps=4)
        clip.write_gif(output_path_gcam, verbose=False)
