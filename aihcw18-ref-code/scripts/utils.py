...
def get_loader(datadir, split):
    ...
    args_dict = {
        'datadir': datadir,
        'batch_size': 16, # change this depending on memory usage
    ...
    _, val_loader, test_loader = load_data(args)
...
def compute_probs_from_objects(model, loader):
    ...
    labels = []
    for batch in loader:
        batch_inputs, label = transform_data(batch, use_gpu=True)
    ...
    return probs_concat, np.concatenate(labels, axis=0)
