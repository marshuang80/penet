...
def get_parser():
    ...
    parser.add_argument('--datadir', type=str, default="/deep/group/aihc-bootcamp-winter2018/medical-imaging/ct_chest_pe/localized")
...
def load_models(model_paths, split):
    ...
    for model in models:
        ...
        model.cuda()
        model.model.register_forward_hook(hook_feature)
...
def get_cams(loader, models, label_names, thresholds, outputdir, save_orig=False):
    ...
    for i, batch in enumerate(loader):
        ...
        img_seq_npy = (img_seq_npy - np.min(img_seq_npy)) / np.max(img_seq_npy)
