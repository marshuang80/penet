...
def getPredMetrics(labels, preds):
    '''
    ...
    return accuracy, f1, kappa, precision, recall, auroc
...
def get_loader(datadir, split):
    ...
    train_loader, val_loader, test_loader, rad_loader, valid2_loader = load_data(args)
    loaders = {'train': train_loader, 'valid': val_loader, 'test': test_loader, 'radio-test-mini': rad_loader, 'valid2': valid2_loader}
...
def get_loader_for_model(model_path, dataset):
    ...
    print("---Loading data")
    train_loader, val_loader, test_loader, rad_loader, valid2_loader = load_data(model_args)
    loaders = {'train': train_loader, 'valid': val_loader, 'test': test_loader, 'radio-test-mini': rad_loader, 'valid2': valid2_loader}
...
def get_model_and_loader(model_path, dataset, view = None):
    # Load model args
    with open(Path(model_path).parent / 'args.json') as args_f:
        model_args_dict = json.load(args_f)
    model_args = Struct(**model_args_dict)
    if view is not None:
        model_args.view = view
    print("---Retrieved model args")
...
def compute_probs_from_objects(model, loader, task):
    ...
    for batch in loader:
        ...
        if loader.dataset.view == 'all':
            batch_logits = model(batch_inputs['sagittal'], batch_inputs['coronal'], batch_inputs['axial'])
        else:
            batch_logits = model(batch_inputs[loader.dataset.view])
        batch_probs = torch.sigmoid(batch_logits)
        batch_probs_npy = batch_probs.cpu().data.numpy()
        task_to_index = {'abnormal':0,'acl':1,'meniscus':2}
        if loader.dataset.task == 'all':
            batch_probs_npy = batch_probs_npy[:,task_to_index[task]]
        probs.append(batch_probs_npy)

    probs_concat = np.concatenate(probs, axis=0)
    probs_concat = np.reshape(probs_concat, (-1,1))
    return probs_concat


def compute_probs_from_paths(model_path, dataset, task):

    model, loader = get_model_and_loader(model_path, dataset)
    
    return compute_probs_from_objects(model, loader, task)
