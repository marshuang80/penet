...
def evaluate(probs, datadir, split, metric, verbose, save_dir='/deep/group/aihc-bootcamp-winter2018/nborus/ct_chest_pe/',n=1):
    ...
    plt.plot(fpr, tpr)
    #print(fpr,tpr)
    plt.plot(0.091, 0.543, 'ro', label="Matt")
    plt.plot(0.091, 0.686, 'go', label="Bhavik")
    save_dir = '{}/{}_ensemble_{}_roc'.format(save_dir,split, n)
    print("Save dir for images: ", save_dir)
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    plt.savefig("{}/roc_orig_groundtruth.png".format(save_dir))
    plt.close()
