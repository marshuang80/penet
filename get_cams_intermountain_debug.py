...
def get_cams(args):
    ...
    for inputs, target_dict in data_loader:
        ...
        with torch.set_grad_enabled(True):
            ...
            #if probs > 0.5:
            #    print("Skipping tru negative")
                #print('Skipping false negatives...')
                #continue
            study2probs[target_dict['study_num']].append(probs.item())
            study2inputs[target_dict['study_num']].append(inputs)
            study2slices[target_dict['study_num']].append(target_dict['slice_idx'])
    for key in study2probs.keys():
        print(key,study2probs[key])
        """
            grad_cam.backward(idx=idx[0])  # Just take top prediction
            cam = grad_cam.get_cam(args.target_layer)
        ...
        # Write to a GIF file
        # TODO
        study_num = target_dict['study_num']
        output_path_input = os.path.join(os.path.join(args.cam_dir, 'false_pos_input{}.gif'.format(num_generated + 1)))
        output_path_cam = os.path.join(args.cam_dir, 'false_pos_output{}.gif'.format(num_generated + 1))
        output_path_combined = os.path.join(args.cam_dir, 'false_pos_combined_{}_{}_{}.gif'.format(num_generated + 1, study_num, probs))
