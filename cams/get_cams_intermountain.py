...
def get_cams(args):
    ...
    for inputs, target_dict in data_loader:
        ...
        # Write to a GIF file
        # TODO
        study_num = str(target_dict['study_num'].data[0])
        if study_num in study_idx_dict:
            study_idx_dict[study_num] += 1
        else:
            study_idx_dict[study_num] = 0
        study_count = study_idx_dict[study_num]
